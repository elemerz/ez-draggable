(function ($) {
    $.fn.ezDraggable = function (opts) {
        ...
        return $el.css('cursor', cfg.cursor).on("mousedown", function (e) {
            var $drag = cfg.handle ? $(this).addClass('active-handle').closest(cfg.draggable).addClass('draggable') : $(this).addClass('draggable');
            ...
            if (cfg.handle) {
                $(this).removeClass('active-handle').closest(cfg.draggable).removeClass('draggable');
            } else {
