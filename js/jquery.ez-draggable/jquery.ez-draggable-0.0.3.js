(function ($) {
    $.fn.ezDraggable = function (opts) {
        var defaults={
                ...
                draggableClass: "ngdialog-content",
        ...
        return $el.css('cursor', cfg.cursor).on("mousedown", function (e) {
            var $drag = cfg.handle ? $(this).addClass('active-handle').closest(cfg.draggableSel).addClass('draggable') : $(this).addClass('draggable');
            ...
            if (cfg.handle) {
                $(this).removeClass('active-handle').closest(cfg.draggableSel).removeClass('draggable');
            } else {
